...
def main():
    ...
    if use_xkv:
        print(f"xKV Groups: {mla_model.mla_config.n_groups} groups of {mla_model.mla_config.cross_layer_group_size} layers")
    if keep_early_original and skip_layers > 0:
        print(f"Early Layers: {skip_layers} layers kept as original (no compression)")
    print(f"\nPERPLEXITY:")
    ...
    print(f"  Effective compression:   {effective_ratio:.2f}x (accounting for {n_original_layers} uncompressed layers)")
